...
- issue #26 安全/可靠性修正：`worker-send` 改用 chunked `bytesToBase64` 避免大型郵件 spread 觸發 call stack，且 webhook 全失敗時在 `waitUntil` catch 內 `message.setReject("Webhook delivery failed")`；Docker/Node/Python 登入端以記憶體 Map 做每 IP 失敗 5 次鎖 60 秒；session 新增 `expiresAt` 並在讀取時過期刪除；production (`NODE_ENV=production`) cookie 加 `Secure`；Python `Store.save()` 以 `threading.Lock` 保護；Node `readBody` 加 15MB 上限；GitHub Storage 會維護 `email_events/index.json` 供列表/count 直接讀取，無 index 時才 fallback 舊 N+1；`web-ui/manifest.json` 升版並加入 `index.html`、`style.css`、`app.js` 的 SHA-256，Docker/Node/Python/Java 熱更新下載後驗證 checksum，不符不寫入快取。
